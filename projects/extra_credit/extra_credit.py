...
def generate_data(snr_dB: float):
    ...
    ----------
    snr_dB : float or int
        This is the signal to noise ratio expressed in dB. This is used to determine the noise standard deviation.
       
    Returns
    -------
    random_bits : np.array
       This includes all of the randomly generated bits. We use this as the answer key.
    sample_times : np.array
       The points in time when we sampled the waveform. This is used in the classical demodulation algorithm.
    dirty_array : np.array
       The sample values of the sinewave we created, with the noise added to it."""
...
def estimate_phase(sample_times: np.array, sample_values: np.array, freq_guesses: np.array):
    ...
    sample_values : np.array
       The sample values of the sinewave we created, with the noise added to it.
    freq_guesses : np.array
       The frequency values that we want to test. The final answer will rely on the correct frequency being in this array.
...
def evaluate_estimate(phase_est: float, correct_bit: int):
    ...
    ----------
    phase_est : float
        The phase estimate at RX.
    correct_bit : int
        The bit that TX wanted to communicate.
...
def demod_classic(random_bits: np.array, sample_times: np.array, dirty_array: np.array):
    ...
    -------
    ber : float
        The bit error rate using the classical demodulation technique."""
...
def demod_knn(random_bits: np.array, dirty_array: np.array, num_neigh=100):
    ...
    dirty_array : np.array
        The sample values of the sinewave we created, with the noise added to it.\n
    num_neigh : int
        The number of neighbors used for the KNN algorithm. Default value is set to 100.\n

    Returns
    -------
    ber: float
        The bit error rate for the machine learning algorithm."""
...
if __name__ == "__main__":
    ...
    random_bits, sample_times, dirty_array = generate_data(snr_dB=0)

    # cl stands for the classical algorithm.
    # ml stands for the machine learning algorithm, KNN in this case.  
    ber_cl = demod_classic(random_bits, sample_times, dirty_array)
    ber_ml = demod_knn(random_bits, dirty_array)
    ...
    plt.scatter(snr_values, ber_vals_cl, label='Classical Method', color='tab:blue')
    plt.scatter(snr_values, ber_vals_ml, label='Machine Learning', color='tab:red')
    plt.xlabel('Signal to Noise Ratio (dB)')
    plt.ylabel('Bit Error Rate (Unitless)')
    plt.title('SNR vs BER')
    plt.legend()
    plt.grid(True)
    plt.savefig('projects/extra_credit/ber_vs_snr.png', dpi=300)
    plt.show()
