...
from matplotlib import animation      # make a movie
...
# now let's make a movie of the positions
...
# Create empty point objects with no data.
# They will be updated with each frame of the animation.
(my_pointb,) = ax.plot([],[],'bo',ms=15)
(my_pointr,) = ax.plot([],[],'ro',ms=20)
(my_pointg,) = ax.plot([],[],'go',ms=25)
...
def get_position(n,x1,y1,x2,y2,x3,y3,pointb,pointr,pointg):
    return (pointb.set_data(x1[n],y1[n]),
            pointr.set_data(x2[n],y2[n]),
            pointg.set_data(x3[n],y3[n]))
    
my_movie = animation.FuncAnimation(fig,get_position,frames=1000,
                                   fargs=(x1,y1,x2,y2,x3,y3,
                                          my_pointb,my_pointr,my_pointg))
my_movie.save('threebody.mp4',fps=30)
