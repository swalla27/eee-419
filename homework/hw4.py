...
# Create the root, then define M (the number of iterations), and N (the number of years until... the end)
root = tk.Tk()
...
# Each entry also gets a default value
mean_return.set('6')
...
# Each entry is placed into tkinter widgets which are placed into the grid in the next step
...
# Now we are finally putting the entry widgets into the root
...
def calculate_fx(mean_return, std_dev_return, yearly_contribution, years_of_contribution, 
    ...
    try:
        ...
        sigma = int(std_dev_return.get()) # The standard deviation of the return, proportional to the noise 
    ...
    for _ in range(M):
        ...
        plt.plot(np.trim_zeros(wealth), marker='x')
    ...
    # I like to have vertical lines showing me when contributions ended and when retirement began
...
tk.Button(root, text='Quit', width=12, command=shut_down_action).grid(row=7, column=0)
